...
<% List<UsuarioDTO> lstUsuarios;
	lstUsuarios= (List) request.getAttribute("listaUsuarios"); %>
<body>
  <div class="container">
    <div id="barraNavegacion">      </div>

    <form action="modificacionExitosa.html">
        <h1>Modificación de Usuario</h1>
        <div class="input-group">
          <select class="custom-select" id="usuarios" name="usuarios">
            <option selected>Seleccione...</option>
          <%for (UsuarioDTO dto : lstUsuarios){ %>
            <option value=<%=dto.getIdUsuario() %>><%=dto.getNombre() %></option>
          <%} %>
          </select>
        </div>
