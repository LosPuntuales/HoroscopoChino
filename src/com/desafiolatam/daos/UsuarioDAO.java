...
public class UsuarioDAO implements CrudDao {
	private final String SQL_SELECT_ALL="SELECT id_usuario, nombre, contrase�a FROM usuarios ";
	private final String SQL_SELECT_ID = "SELECT id_usuario, nombre, contrase�a FROM usuarios WHERE id_usuario = ?";
	private final String SQL_INSERT = 	"INSERT INTO usuarios ( nombre, contrase�a) VALUES (?,?)";
	private final String SQL_UPDATE = "UPDATE usuarios SET nombre = ?, contrase�a=? WHERE id_usuario = ?";
	...
	public List selectAll() {
		...
		try{
			...
			while(resultado.next()) {
				...
				usuarioDTO.setPass(resultado.getString("contraseña"));
	...
	public Object selectById(Object obj) {
		//creamos objeto que devolver� el usuario
		UsuarioDTO usuarioDTO = (UsuarioDTO) obj;
		//conexion a la base de datos y ejecucion de la sentencia
		Connection conexion = Singleton.getConnection();
		try{
			...
			while(rs.next()) {//Traspasa los valores de la consulta al objeto a retornar
				...
				usuarioDTO.setPass(rs.getString("contraseña"));
			}
		}catch(Exception ex) {
			ex.printStackTrace();
		}
